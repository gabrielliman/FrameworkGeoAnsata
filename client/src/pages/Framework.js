...
function Framework() {
    ...
    useEffect(() => {
      axios.get(`http://localhost:3001/frameworks/byId/${framework_id}`,{ withCredentials: true }).then((response) => {
          setFrameworkObject(response.data)
      });

      axios.get(`http://localhost:3001/sections/${framework_id}`,{ withCredentials: true }).then((response) => {
  ...
  <div>
    ...
    <div>
        {listOfSections.map((value, key) => {
            return (
                <div className="Section" key={value.ID} onClick={()=>{navigate(`/section/${value.ID}`)}}>
