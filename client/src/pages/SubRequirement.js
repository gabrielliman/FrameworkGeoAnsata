...
function SubRequirement() {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3001/subrequirements/byId/${subrequirement_id}`, { withCredentials: true })
      .then((response) => {
        setSubRequirementObject(response.data);
      });

    axios.get(`http://localhost:3001/questions/${subrequirement_id}`, { withCredentials: true })
      .then((response) => {
        axios.post("http://localhost:3001/referencequestions/byIds", response.data)
          .then((response) => {
  ...
  const handleSubmit = () => {
    ...
    if (isEmpty) {
      ...
      axios.post("http://localhost:3001/answers", submission)
        .then(response => {
  ...
  return (
    <div>
      ...
      <button className="submit-button" onClick={handleSubmit}>Submit Response</button>
