...
const { Requirements, SubRequirements, Questions, Answers, Instances } = require('../models');

// GET route to fetch all questions of subrequirements for a specific requirement and instance along with answer statistics
router.get('/requirements/:requirementID/instances/:instanceID/questions-answers', async (req, res) => {
  ...
  try {
    ...
    const requirement = await Requirements.findByPk(requirementID, {
      include: {
        ...
        where: {
          Class: {
            [Sequelize.Op.in]: ['Exploration', 'Resource', 'Reserve'].slice(0, ['Exploration', 'Resource', 'Reserve'].indexOf(instance.Class) + 1)
          }
